...
Candidate values are proposed by adding Gaussian noise to each parameter independently: the first three parameters are perturbed using N(0, 0.05) and the price parameter using N(0, 0.005). We ran the sampler for 11,000 iterations and discarded the first 1,000 as burn-in.
...
# Proposal function: Gaussian noise added to each dimension
...
run_mcmc <- function(start, iterations = 11000) {
...
# Run the MCMC
start_vals <- rep(0, 4)
mcmc_chain <- run_mcmc(start = start_vals, iterations = 11000)

# Drop burn-in
mcmc_chain_post <- mcmc_chain[1001:11000, ]
colnames(mcmc_chain_post) <- colnames(X)

# Show summary of posterior means and 95% credible intervals
