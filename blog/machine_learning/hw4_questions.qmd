...
# Optional: Show summary statistics for the clustering variables
...
k_means_custom <- function(data, k = 3, max_iter = 10) {
  ...
  for (iter in 1:max_iter) {
    ...
    # Update clusters FIRST so history records correct labels
    clusters <- new_clusters

    # Store for plotting
    history[[iter]] <- list(centers = centers, clusters = clusters)
...
for (i in seq_along(k_result$history)) {
  cluster_df <- penguins_clean %>%
    mutate(cluster = factor(k_result$history[[i]]$clusters))

  centers_df <- as.data.frame(k_result$history[[i]]$centers)

  p <- ggplot(cluster_df, aes(x = bill_length_mm, y = flipper_length_mm, color = cluster)) +
    geom_point(size = 2, alpha = 0.7) +
    geom_point(data = centers_df, aes(x = bill_length_mm, y = flipper_length_mm),
               shape = 8, size = 4, color = "black") +
    labs(title = paste("K-means Iteration", i),
         x = "Bill Length (mm)",
         y = "Flipper Length (mm)")
...
penguins_clustered <- penguins_clean %>%
  mutate(cluster = factor(k_result$final_clusters))

centers_final <- as.data.frame(k_result$centers)

ggplot(penguins_clustered, aes(x = bill_length_mm, y = flipper_length_mm, color = cluster)) +
  geom_point(size = 2, alpha = 0.8) +
  geom_point(data = centers_final, aes(x = bill_length_mm, y = flipper_length_mm),
             shape = 8, size = 4, color = "black") +
  labs(title = "Final K-means Clustering with k = 3",
       x = "Bill Length (mm)",
       y = "Flipper Length (mm)",
       color = "Cluster") +
  theme_minimal()
...
Our custom K-means implementation identified 3 well-separated clusters in the penguins dataset. Visualization of each step shows how the algorithm converges toward stable centroids.
...
We implemented the K-nearest neighbors (KNN) algorithm from scratch using Euclidean distance to classify new observations based on their k = 5 closest training points. Our custom implementation achieved an accuracy of about `r round(mean(pred_custom == test_y), 2)` on the test data. To validate our results, we used R’s built-in class::knn() function, which gave the same prediction accuracy — confirming that our implementation works correctly.
...
final_accuracy
```
### Final KNN Classification Accuracy
