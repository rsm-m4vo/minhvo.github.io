...
We begin by loading the dataset provided by the authors. The dataset is stored in the `data/` directory and contains variables related to donor demographics, past behavior, and treatment assignments.
...
# T-test and regression for months since last donation
```{r}
with(df, t.test(mrm2 ~ treatment))
...
ggplot(prop_data, aes(x = factor(treatment), y = rate, fill = factor(treatment))) +
  ...
  labs(x = "Group", y = "Proportion Donated", fill = "Group") +
  ggtitle("Donation Rate by Treatment Group") +
  theme_minimal()

# T-test and linear regression
with(df, t.test(gave ~ treatment))
...
```{r}
# Compare 1:1 vs 2:1 match
with(df, t.test(gave[ratio2 == 1], gave[ratio == 1]))
# Compare 2:1 vs 3:1 match
with(df, t.test(gave[ratio3 == 1], gave[ratio2 == 1]))
