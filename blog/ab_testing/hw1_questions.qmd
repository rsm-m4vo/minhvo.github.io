...
```{r}
modelsummary(lm(mrm2 ~ treatment, data = df))
```
...
# Linear regression
modelsummary(lm(gave ~ treatment, data = df))
```
...
```{r}
modelsummary(lm(gave ~ ratio2 + ratio3, data = df))
```
...
```{r}
modelsummary(lm(amount ~ treatment, data = df))
```
...
modelsummary(lm(amount ~ treatment, data = df_givers))
```
